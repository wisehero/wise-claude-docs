...
> **난이도**: 심화 | **선행 문서**: [Claude Code 시작하기](01-시작하기.md) | **최초 작성**: 2026-03-31 | **마지막 수정**: 2026-05-12
>
...
Claude Code는 약 30개의 이벤트에 Hook을 걸 수 있다. 전체 최신 목록은 [공식 hooks 문서](https://code.claude.com/docs/en/hooks)를 참고한다. 아래는 자주 쓰이는 이벤트들을 카테고리별로 정리한 것이다.
...
| `Setup` | `--init-only` 또는 `-p` 모드의 `--init`/`--maintenance` 실행 시 | 아니오 |
...
| 이벤트 | 발생 시점 | 차단 가능 여부 |
|---|---|---|
| `UserPromptSubmit` | 사용자 메시지 제출 시 | 예 |
| `UserPromptExpansion` | 슬래시 명령이 확장될 때 | 예 |
...
| `PermissionDenied` | auto 모드 분류기가 도구를 거부했을 때 | 재시도 가능 |
| `PostToolUse` | 도구 실행 성공 후 | 아니오 |
| `PostToolUseFailure` | 도구 실행 실패 후 | 아니오 |
| `PostToolBatch` | 병렬 도구 호출 배치가 모두 끝난 후 | 아니오 |

**응답·서브에이전트**

| 이벤트 | 발생 시점 | 차단 가능 여부 |
|---|---|---|
| `Stop` | Claude 응답 완료 시 | 예 (조건 충족 시까지 stop 차단 가능) |
| `StopFailure` | API 에러로 응답이 끝난 경우 | 아니오 |
...
| 이벤트 | 발생 시점 | 차단 가능 여부 |
|---|---|---|
| `TaskCreated` | 태스크 생성 시 | 아니오 |
| `TaskCompleted` | 태스크 완료 시 | 아니오 |
| `PreCompact` | 대화 압축 직전 | 예 |
| `PostCompact` | 대화 압축 직후 | 아니오 |

이 외에도 `ConfigChange`, `CwdChanged`, `FileChanged`, `InstructionsLoaded`(CLAUDE.md/`.claude/rules/*.md` 로드 시), `TeammateIdle`, `WorktreeCreate`, `WorktreeRemove`, `Elicitation`/`ElicitationResult`(MCP 서버의 사용자 입력 요청) 등의 이벤트가 있다. Hook 이벤트는 Claude Code 업데이트에 따라 추가되므로 공식 문서를 기준으로 확인한다.

차단 가능한 이벤트(`PreToolUse`, `PermissionRequest`, `UserPromptSubmit` 등)에서는 Hook이 종료 코드 `2`를 반환하면 해당 작업이 실행되지 않는다. 차단이 불가능한 이벤트는 알림·로깅 용도로 사용하며, Hook의 반환값이 작업 실행 여부에 영향을 주지 않는다.
...
Hook에서 실행할 작업의 형태는 다섯 가지다(`command`, `http`, `mcp_tool`, `prompt`, `agent`). `agent`는 실험적 타입이다.
...
MCP 서버에 등록된 도구를 Hook에서 직접 호출한다. 정책 검증·외부 시스템 조회처럼 외부 도구로 결정을 위임할 때 적합하다.

```json
{
  "type": "mcp_tool",
  "server": "policy-engine",
  "tool": "check_edit_allowed"
}
```
...
Hook은 두 가지 방법으로 결정을 표현할 수 있다.

**1) 종료 코드**

| 종료 코드 | 의미 |
|---|---|
| `0` | 정상 처리. 작업을 진행한다. stdout이 JSON이면 파싱한다. |
| `2` | 차단(blocking error). stderr를 Claude에 전달한다. `PreToolUse`, `PermissionRequest`, `Stop`, `UserPromptSubmit` 등에서 효과적이다. |
| 그 외 | 비차단 오류로 처리. 대부분 이벤트에서 작업은 진행된다. |

**2) JSON 결정(`hookSpecificOutput`)**

차단/허용을 더 명시적으로 제어하려면 stdout으로 JSON을 출력한다. 이벤트별로 허용되는 필드가 다르다.

```json
// PreToolUse — allow / deny / ask / defer
{
  "hookSpecificOutput": {
...
// UserPromptSubmit / PostToolUse / Stop 등 — 최상위 decision 사용
{
  "decision": "block",
  "reason": "이 프롬프트 패턴은 허용되지 않습니다",
  "hookSpecificOutput": {
    "hookEventName": "UserPromptSubmit",
    "additionalContext": "Claude에 추가로 제공할 컨텍스트"
  }
}
```

`PreToolUse`·`PermissionRequest`는 `permissionDecision`에 `"allow" | "deny" | "ask" | "defer"`를 지정할 수 있고, 그 외 차단 가능 이벤트는 최상위 `decision: "block"`을 쓴다. JSON 결정 방식이 종료 코드 `2`보다 이유 전달과 후속 제어가 명확하다.

### stdout으로 메시지 전달

Hook이 표준 출력(stdout)으로 내보내는 텍스트는 Claude Code가 읽어 컨텍스트에 포함한다(JSON이면 결정으로 해석되고, 그 외 텍스트는 Claude에 전달된다). 차단 이유나 추가 정보를 전달할 때 활용한다.
...
- Hook 이벤트·결정 API 전체 레퍼런스 -> [Claude Code 공식 hooks 문서](https://code.claude.com/docs/en/hooks)
- 권한 설정 세부 옵션 -> [Claude Code 공식 settings 문서](https://code.claude.com/docs/en/settings)
