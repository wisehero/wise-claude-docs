...
아래는 자주 사용되는 주요 이벤트다. 전체 목록(25개 이상)은 [공식 문서](https://docs.anthropic.com/en/docs/claude-code/hooks)를 참고한다.
...
`PreToolUse`, `PermissionRequest`, `UserPromptSubmit` 등 차단 가능한 이벤트에서 Hook이 종료 코드 `2`를 반환하면 해당 작업이 실행되지 않는다.
...
글로벌 설정(`~/.claude.json`)은 모든 프로젝트에, 프로젝트 설정(`.mcp.json`)은 해당 프로젝트에만 적용된다.
