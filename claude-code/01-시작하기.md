...
> **난이도**: 기초 | **선행 문서**: 없음 | **최초 작성**: 2026-03-31 | **마지막 수정**: 2026-05-12
>
...
설치 스크립트가 Claude Code 바이너리를 내려받아 PATH에 등록한다. 완료 후 새 터미널 탭을 열거나 `source ~/.bashrc`(또는 `~/.zshrc`)를 실행해 PATH를 갱신한다. **네이티브 설치는 백그라운드에서 자동으로 최신 버전을 받아오므로 별도 업데이트 명령이 필요 없다.**

Homebrew를 선호한다면 두 가지 cask 중에서 선택한다.

```bash
brew install --cask claude-code           # stable 채널 (약 1주 지연, 회귀 있는 릴리스는 건너뜀)
brew install --cask claude-code@latest    # latest 채널 (출시 즉시 반영)
```

Homebrew 설치본은 자동 업데이트가 지원되지 않는다. 최신 버전을 유지하려면 주기적으로 `brew upgrade claude-code`(또는 `brew upgrade claude-code@latest`)를 실행해야 한다.

Debian/Ubuntu, Fedora/RHEL, Alpine 환경이라면 apt/dnf/apk 패키지 매니저로도 설치할 수 있다. 자세한 명령은 [공식 setup 가이드](https://code.claude.com/docs/en/setup#install-with-linux-package-managers)를 참고한다.
...
명령 프롬프트(CMD)에서 설치하려면 아래 명령을 사용한다.
...
Homebrew와 마찬가지로 WinGet 설치본도 자동 업데이트가 지원되지 않는다. `winget upgrade Anthropic.ClaudeCode`로 직접 갱신한다.

**Windows 사용 시 주의 사항**: 네이티브 Windows에서 Claude Code의 Bash 도구를 쓰려면 [Git for Windows](https://git-scm.com/downloads/win)를 설치하는 것을 권장한다. 설치되어 있지 않으면 Claude Code는 셸 도구로 PowerShell을 대신 사용한다. WSL 환경에서는 Git for Windows가 필요하지 않다.
...
| Claude Console (API) | Anthropic API 키를 통한 직접 접근. 첫 로그인 시 Console에 "Claude Code" 워크스페이스가 자동 생성되어 비용 추적이 일원화된다 |
| Amazon Bedrock | AWS에서 호스팅하는 Claude 모델 |
| Google Vertex AI | GCP에서 호스팅하는 Claude 모델 |
| Microsoft Foundry | Azure에서 호스팅하는 Claude 모델 |

Pro 또는 Max 구독자라면 브라우저 OAuth(개방형 인증) 흐름으로 로그인한다. API 키를 사용하는 경우 Console에서 발급한 키를 입력한다. Bedrock, Vertex AI, Microsoft Foundry는 각 클라우드 제공업체의 자격증명 설정이 선행되어야 한다.
...
REPL(Read-Eval-Print Loop, 읽기-평가-출력 반복) 환경이 열린다. 여기서 자연어로 요청을 입력하면 Claude가 파일을 읽고, 코드를 작성하고, 명령을 실행한다. 세션을 종료하려면 `exit`를 입력하거나 `Ctrl+D`(또는 `Ctrl+C` 두 번)를 누른다.
...
자연어로 직접 부탁하면 된다. Claude Code는 별도의 `claude commit` 서브명령을 갖지 않으며, 대신 대화 중 다음과 같이 요청한다.

```text
내 변경 사항을 의미 있는 메시지로 커밋해줘
```

Claude가 `git status`/`git diff`로 변경 내역을 확인하고 적절한 커밋 메시지를 작성한 뒤 커밋을 만든다.
...
| `/login` | 계정 로그인 |
| `/resume` | 이전 대화 목록에서 선택해 재개(`claude -r`과 동일) |
| `exit` | 세션 종료 |
