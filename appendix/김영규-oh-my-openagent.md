...
GSD/BMAD/Spec-Kit 같은 "프로세스 통째로 가져가는" 프레임워크가 아니라, **다른 모델·다른 도구의 검증된 패턴을 한 하네스 안에 흡수**한 결과물이다. AmpCode의 딥 모드, Claude Code의 훅/스킬/MCP 호환성, [Can Bölük의 oh-my-pi에서 가져온 해시 기반 편집 도구](https://blog.can.ac/2026/02/12/the-harness-problem/), [맷 포콕 시리즈](맷-포콕-스킬-모음.md)의 스킬 패턴 등을 한 곳에 모았다.
...
시니어 엔지니어가 쓴 것과 구별되지 않아야 한다는 명시적 기준. "초안", "출발점"이 아니라 그대로 머지 가능한 상태가 목표다.
...
같은 입력 → 같은 동작, 세션 재개 가능, 일일이 챙기지 않아도 위임 가능. 궁극적으로 "보이지 않는 인프라(invisible infrastructure)"가 되는 것이 목표다.

이 다섯 항목은 [보리스 체르니 인사이트](보리스-체르니-인사이트.md)의 "검증 가능성이 품질을 2–3배 끌어올린다"와 정확히 같은 방향이다. 단, 체르니가 Claude Code 내부 설계자 관점에서 말한 반면, 김영규는 외부 하네스 작성자 관점에서 같은 결론에 도달했다.
...
| **Hephaestus** | `gpt-5.5` | 자율 심층 작업자. "The Legitimate Craftsman(정당한 장인)" — AmpCode 차단 사건에 대한 반어 |
| **Prometheus** | `claude-opus-4-7` 계열 | 계획자. 인터뷰 모드로 범위를 뽑아낸다 |
| **Atlas** | — | Prometheus 계획을 받아 서브에이전트에 분배하는 실행 지휘자 |
| **Oracle** | — | 읽기 전용 아키텍처/디버깅 자문역 |
| **Librarian** | — | 멀티 레포 및 OSS 구현 검색 |
| **Explore** | — | 빠른 코드베이스 검색 |
| **Multimodal-Looker** | — | PDF/이미지/다이어그램 분석 |
| **Metis / Momus** | — | 계획 전 누락 분석 / 계획 검증 |
...
위임할 때는 카테고리만 적고, 매핑이 모델로 자동 해석된다. 이 분리(decoupling)는 모델이 매달 바뀌는 현실에 대한 응답이다. 매니페스토의 "단일 프로바이더가 시장을 지배하지 못한다(No single provider will dominate)"가 이 설계로 구현됐다.
...
에이전트 진영 중 가장 특이한 캐릭터다. Sisyphus가 "오케스트레이터", Prometheus가 "계획자"라는 명확한 분업을 갖는다면, Hephaestus는 **AmpCode의 딥 모드를 OpenCode 쪽으로 이식한 자율 작업자**다. AmpCode 차단 사건과 맞물려 "The Legitimate Craftsman(정당한 장인)"이라는 별명이 의도된 반어로 붙었다.
...
| `maxTokens` | 32,000 | 한 응답의 최대 출력 크기 |
...
| `permission.call_omo_agent` | `deny` | **다른 OMO 에이전트(Sisyphus 등) 재귀 호출 금지** |

`call_omo_agent: deny`가 중요하다. Hephaestus는 Sisyphus의 부하가 아니라 **혼자 끝내야 하는 자율 작업자**다. 다른 에이전트로 다시 위임할 수 없다. 대신 `task(subagent_type="explore"|"librarian"|"oracle")`와 카테고리 위임은 허용된다. 즉 조사·자문은 위임하되, 책임은 자기가 진다.
...
| `gpt-5.4` (Sisyphus 네이티브) | `gpt-5-4.ts` | XML 태그 8개 섹션, 엔트로피 감소형 |
| `gpt-5.3-codex` | `gpt-5-3-codex.ts` | 549줄, 작업 규율 강화 |
| 그 외 GPT | `gpt.ts` | 507줄 기본 프롬프트 |
...
"코드를 제안하지 말고 구현하라"가 기본값이다. Claude Code의 기본값(보수적·대화적)과 정반대다.

> "Status requests are not stop signals. Give the update, then keep working."

상태 질문에 답하느라 작업을 멈추지 않는다. "어디까지 됐어?"는 진행 종료 신호가 아니라 진행 중에 응답하는 보조 채널이다.

#### 4.3.2 Intent Extraction Table(의도 추출 표) — 표면 발화 vs 진짜 의도

프롬프트 본문에 의도 추출 표가 박혀 있다.
...
#### 4.3.3 Discovery & Retrieval(탐색·조회) — "한 번 넓게, 그다음에 좁게"
...
처음 한 번은 2~5개를 병렬로 동시 호출하고, 그 이후엔 답이 부족하거나, 2차 질문이 떴거나, 결정에 필요한 특정 문서가 있을 때만 추가 호출. 그리고:
...
#### 4.3.4 Manual QA Gate(수동 QA 관문) — 가장 특이한 설계
...
`lsp_diagnostics` 통과 / 빌드 통과 / 테스트 통과 — 이건 다 "증거"일 뿐 "관문"이 아니라고 못박는다. 진짜 관문은 자기 손으로 산출물을 그 표면(surface)으로 한 번 굴려보는 것이다.

| 산출물 표면 | 강제 검증 방법 |
|------------|---------------|
| TUI / CLI / 셸 바이너리 | `interactive_bash`(tmux)에서 키 입력, 정상 경로, 잘못된 입력 1회, `--help` 확인 |
| 웹 / 브라우저 UI | `playwright` 스킬 로드, 실제 브라우저에서 클릭·폼·콘솔·스크린샷 |
| HTTP API / 서비스 | `curl` 또는 드라이버 스크립트로 실프로세스 호출 |
| 라이브러리 / SDK / 모듈 | 임포트해서 처음부터 끝까지 돌리는 최소 드라이버 스크립트 작성 |
...
#### 4.3.5 Three-Attempt Failure Protocol(3회 시도 실패 절차)
...
세 번 시도해서 다 실패하면 — 편집 즉시 중단, 알려진 정상 상태로 되돌리기, 시도 기록, Oracle 동기 호출, 그래도 안 되면 사용자에게 정확히 한 가지만 질문. 단순한 "포기 조건"이 아니라 **상태 되돌리기를 강제**하는 구조다. 이건 "에이전트 슬롭(agent slop)"의 가장 흔한 실패 모드 — 점점 더 망가뜨리며 작업을 계속하는 것 — 을 끊는다.

#### 4.3.6 Hard Invariants(절대 불변 조항) — 무조건 금지
...
매니페스토 1번("인간 개입 = 실패 신호")이 자율성을 최대화하는 방향이라면, Hard Invariants는 그 자율성에 상한선을 친다. "출시 압박에도 절대 양보 못 함"이라는 표현이 그대로 박혀 있다.

### 4.4 운영 루프 — 탐색 → 계획 → 구현 → 검증 → 수동 QA

> "Loops are short and tight; do not loop back with a draft when the work is yours to do."

5단계가 명시되고, 각 단계 사이에 "초안을 들고 사용자에게 돌아가지 마라"가 박혀 있다. 즉 계획 단계 후 사용자 확인을 받지 않고 바로 구현하는 것이 기본값이다. Prometheus(인터뷰형 계획자)와 명확히 다른 포지션 — Prometheus는 작업 전에 사람과 대화하고, Hephaestus는 작업 중에 사람을 부르지 않는다.
...
**적합**:
- 구현 전 깊은 탐색이 필요한 작업
- 사용자가 처음부터 끝까지 자율 완료를 원할 때
- 여러 파일에 걸친 복합 변경 작업

**부적합**:
- 단일 단계의 단순 작업 (과잉 투입, 비용만 비쌈 — `quick` 카테고리가 적합)
- 단계마다 사용자 확인이 필요한 작업 (Hephaestus는 안 멈춤)
- 여러 에이전트 간 오케스트레이션이 필요할 때 (Atlas 또는 Sisyphus가 적합)

`cost: EXPENSIVE` 라벨이 붙은 이유는 GPT-5.5 토큰 가격이 아니라 완료까지 안 멈추는 성향 때문이다. 잘못된 작업에 붙이면 비용이 폭발한다.
...
- **Intent Extraction Table을 프롬프트에 박는 패턴**. 표면 표현과 진짜 의도의 매핑을 표로 명시하는 것은, 모델한테 "행간 읽기"를 결정론적으로 강제하는 좋은 방법이다. 한국어 도메인이라면 "한번 봐줄래?", "혹시 가능하면", "되면 좋을 텐데" 같은 한국어 특유의 완곡 표현 매핑이 강력한 후보다.
- **Three-Attempt Protocol의 되돌리기 강제**. 단순 "포기 조건"이 아니라 상태 복구까지 묶어둔 점이 특히 좋다. 시도 누적 → 되돌리기 → 자문 → 질문의 4단 절차는 일반 스킬에도 그대로 이식 가능하다.
...
`ultrawork`, `search`, `analyze`, `team` 같은 키워드를 단순 정규식이 아니라 LLM 기반 의도 분석으로 분류한다. "문자 그대로의 오독(literal misinterpretation)"을 피하는 게 목표.
...
Ralph Loop은 출력에 `<promise>DONE</promise>` 토큰이 등장할 때까지 같은 프롬프트를 재투입하는 단순한 자기참조 루프다. `/ulw-loop`은 그 위에 병렬 에이전트와 공격적 탐색을 얹은 완전판이다.

핵심은 모델이 "끝났다"고 자기 선언할 때까지 루프가 끊기지 않는다는 점이다. **Todo Enforcer**가 보조 역할로, 에이전트가 유휴 상태로 빠지면 강제로 컨텍스트를 되돌려준다.
...
기본은 비활성. 활성화하면 `team_create`/`team_send_message`/`team_task_create` 등 12개의 `team_*` 도구가 잠금 해제된다. 리드 에이전트가 최대 8명(병렬 4명)의 카테고리 특화 멤버에게 메일함 기반 비동기 통신으로 작업을 분배한다. tmux로 모든 멤버의 상황을 실시간으로 본다.
...
- `hyperplan` — 적대적 비평가 5명이 계획을 직교 각도에서 분해
- `security-research` — 취약점 헌터 3명 + 익스플로잇 검증 엔지니어 2명이 코드베이스를 병렬 감사

제약: 멤버당 500턴, 메시지 본문 32KB, 실행 최대 120분, 메시지 10,000개.
...
스킬이 자기 MCP 서버를 SKILL.md의 frontmatter(메타데이터 헤더)에 선언한다. 스킬이 활성화될 때만 MCP가 뜨고, 끝나면 사라진다. 컨텍스트 윈도우에 MCP 도구가 상주하지 않는다는 게 핵심.

이 가이드의 [커스텀 스킬 문서](../claude-code/03-커스텀-스킬.md)에서 다룬 스킬 구조를 [MCP 서버 연동](../claude-code/05-MCP-서버-연동.md)과 결합한 형태로, **범위를 작업 단위로 좁힌** 변형이다.
...
API 오류(429/503/529) 시 자동으로 다음 모델로 전환한다. `fallback_models`는 단순 문자열 배열과 모델별 객체 설정을 한 배열에 섞어쓸 수 있고, 모델별 휴지 시간(cooldown) 설정도 가능하다. Sisyphus가 Opus/Kimi/GLM 사이에서 무중단으로 움직이는 이유다.
...
`task(category: "ultrabrain", prompt: ...)` 같은 형태로 위임 시점에 모델을 직접 지정하지 않는다. 매월 모델이 바뀌는 현실에서, 호출부와 매핑부를 분리해두면 라우팅 정책만 바꿔서 전체를 옮길 수 있다. 본 가이드의 [모델 선택 가이드](../claude/04-모델-선택-가이드.md)가 사용자 의사결정용이라면, OMO는 같은 의사결정을 시스템 계층으로 끌어올렸다.

### 6.2 도구 신뢰성 = 모델 능력의 곱셈 상수

Hashline의 6.7% → 68.3% 사례가 단적이다. 같은 모델, 같은 프롬프트, 도구만 교체. 새 스킬이나 새 프롬프트를 짜기 전에, 기존 도구 인터페이스가 모델한테 "안전하게 실패하는" 형태인지 점검하는 게 투자 대비 효과가 훨씬 크다.

### 6.3 자기참조 루프와 강제 복귀를 한 쌍으로 둔다

Ralph Loop("끝났다고 자기선언할 때까지 돌린다") 단독은 무한 루프 위험이 있다. OMO는 Todo Enforcer("유휴 상태 감지 시 컨텍스트 복귀")와 짝으로 둔다. 매니페스토 5번 항목("컴파일러처럼 예측 가능")이 이렇게 구현됐다.

### 6.4 카테고리화된 적대적 검토 팀을 미리 만들어둔다

`hyperplan`의 "적대적 비평가 5명" 패턴이 흥미롭다. 일반화하면 — 계획 단계에 직교 관점의 비평가 N명을 자동 호출하는 스킬은 어느 도메인에서나 적용 가능하다. 한국어 도메인이라면 보안·PM·QA·성능·접근성 5각도 비평가 정도.
...
- **벤더 중립을 표방하지만 OpenCode에 강하게 묶여 있다.** OpenCode 플러그인 API와 훅 시스템을 그대로 쓴다. OpenCode가 죽으면 OMO도 죽는다. "Claude에도, OpenAI에도 매여 있지 않다(Not locked to Claude. Not locked to OpenAI.)"는 사실이지만, OpenCode에는 잠겨 있다.
- **"Anthropic이 OpenCode를 차단했다" 서사는 1차 출처가 트윗이다.** Anthropic 공식 발표가 아니라 OpenCode 메인테이너의 X 게시물이 근거다. 마케팅 카피로는 강하지만, 인과관계를 단정할 만큼 검증된 사실은 아니다.
- **벤치마크 숫자 출처가 단일하다.** 6.7% → 68.3%는 oh-my-pi의 원 측정치를 인용하는 형태이고, OMO가 자체 측정한 숫자는 공개되지 않았다. 도구 교체가 큰 효과라는 방향성은 합리적이지만, 절대 수치는 다른 모델·작업에서 그대로 재현되지 않을 수 있다.
- **Team Mode는 기본이 비활성이고 제약이 많다.** 120분 / 8명 / 500턴 / 32KB. 진짜 멀티에이전트라기보단 "오케스트레이션이 잘 도구화된 서브에이전트" 수준에 가깝다. 멀티에이전트가 단일 에이전트보다 항상 낫다는 증거는 아직 없다는 점도 기억할 만하다.
- **`ultrawork` 같은 "한 마디로 끝" 표현은 잘 작동하는 경우가 한정적이다.** README 인용 사례들(eslint 경고 8,000개, 4만 5천 줄 tauri → SaaS)은 모두 구조가 단순하고 검증 가능한 작업이다. 도메인 모델 설계나 비기능 요구사항이 얽힌 작업에서도 같은 결과가 나온다는 보장은 없다.
...
- **Hashline 패턴** — 한국어 도메인 한정 스킬을 만들 때, `Read`/`Edit` 도구 인터페이스를 그대로 쓰지 말고 해시 검증 계층을 한 겹 두르는 게 투자 대비 효과가 크다. 이 가이드의 [하네스 엔지니어링](../claude-code/08-하네스-엔지니어링.md) 사례 모음에 추가 가능.
...
- **Sisyphus/Hephaestus 같은 캐릭터 이름** — OMO 내부에서는 일관된 비유지만, 다른 레포에 옮기면 단순히 멋스러운 이름짓기에 그칠 위험이 크다. 본 가이드의 기존 스킬 이름 규칙(`refactor-advisor`, `spec-writer` 같은 기능 서술형)을 유지하는 게 발견 가능성이 높다.
- **Team Mode 자체** — 제약(120분/8명)이 강하고, Claude Code/Codex CLI 양쪽 모두 자체 서브에이전트 시스템이 이미 있다. OMO 전용 인프라에 의존하지 말고, [실전 워크플로우 문서](../claude-code/06-실전-워크플로우.md)의 멀티에이전트 패턴을 활용하는 게 낫다.
