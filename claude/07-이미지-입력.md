...
> **난이도**: 중급 | **선행 문서**: [프롬프트 기초](01-프롬프트-기초.md) | **최초 작성**: 2026-03-31 | **마지막 수정**: 2026-04-22
>
...
공식 [Vision 문서](https://platform.claude.com/docs/en/build-with-claude/vision) 기준이다.

| 구분 | 제한 |
|---|---|
| claude.ai | 메시지당 최대 **20장** |
| API (200K 컨텍스트 모델) | 요청당 최대 **100장** |
| API (그 외 모델) | 요청당 최대 **600장** |
| 표준 엔드포인트 요청 크기 | **32MB** |

- 최대 해상도: 명시적 상한은 없으며, 내부적으로 적절한 크기로 리사이즈된다.
...
**클립보드 붙여넣기**: `Ctrl+V`(또는 iTerm2의 `Cmd+V`, Windows의 `Alt+V`)로 클립보드의 이미지를 대화에 직접 첨부한다. 커서 위치에 `[Image #N]` 참조가 삽입되어 프롬프트 안에서 위치 기반으로 언급할 수 있다. 공식 [Interactive mode](https://code.claude.com/docs/en/interactive-mode) 문서에 명시된 동작이다. 스크린샷을 찍고 바로 붙여넣는 패턴이 가장 자주 쓰인다.

**파일 경로 지정**: 대화에서 이미지 파일의 절대/상대 경로를 전달하면 Claude Code가 `Read` 도구로 해당 파일을 읽어 분석할 수 있다. 경로만 단독으로 쓰기보다 "이 이미지를 보고 ~해줘"처럼 요청을 명시하는 편이 안정적이다(공식 문서에는 "경로만 언급하면 자동 분석된다"는 동작이 명시되어 있지 않다).
