...
> **난이도**: 중급 | **선행 문서**: [프롬프트 기초](01-프롬프트-기초.md) | **최초 작성**: 2026-03-31 | **마지막 수정**: 2026-03-31
>
...
**중요: Claude 4.6 이후 모델(Opus 4.7, Opus 4.6, Sonnet 4.6)에서는 마지막 assistant 턴의 prefilling이 더 이상 지원되지 않는다.** 대화 중간에 assistant 메시지를 삽입하는 것은 여전히 가능하지만, 응답 시작을 강제하는 용도의 prefilling은 4.6 이전 모델에서만 유효하다. Opus 4.7에서는 prefilling 대신 structured outputs나 `output_config.format`으로 출력 형식을 강제하는 것이 권장 방식이다.
...
이 문서에서 다룬 기법들은 모두 단일 대화 턴 안에서 작동하는 기법들이다. 시스템 프롬프트를 활용해 Claude의 행동과 성격을 정의하는 방법은 `03-시스템-프롬프트-설계.md`에서, Claude Code의 실전 워크플로우는 `claude-code/06-실전-워크플로우.md`에서 다룬다.
