...
**중요: Claude 4.6 모델(Opus 4.6, Sonnet 4.6)에서는 마지막 assistant 턴의 prefilling이 더 이상 지원되지 않는다.** 대화 중간에 assistant 메시지를 삽입하는 것은 여전히 가능하지만, 응답 시작을 강제하는 용도의 prefilling은 4.6 이전 모델에서만 유효하다.
