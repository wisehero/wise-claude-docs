...
이 문서에서 다룬 기법들은 모두 단일 대화 턴 안에서 작동하는 기법들이다. 더 긴 작업을 여러 대화에 걸쳐 처리하거나, Claude가 스스로 도구를 사용하며 작업을 진행하는 에이전트 패턴에 대해서는 시스템 프롬프트 설계 `03-system-prompt-design.md`에서 다룬다.
