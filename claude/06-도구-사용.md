...
> **난이도**: 중급 | **선행 문서**: [프롬프트 기초](01-프롬프트-기초.md), [시스템 프롬프트 설계](03-시스템-프롬프트-설계.md) | **최초 작성**: 2026-03-31 | **마지막 수정**: 2026-05-12
>
...
### Client tool vs Server tool

도구는 어디서 실행되느냐에 따라 두 종류로 나뉜다.

- **Client tool**: 사용자가 정의하거나 Anthropic 스키마를 따르는 도구(`bash`, `text_editor` 등)가 여기 속한다. Claude가 `stop_reason: "tool_use"`와 함께 `tool_use` 블록을 돌려주고, 개발자의 코드가 실행한 뒤 `tool_result`로 돌려준다.
- **Server tool**: Anthropic 인프라에서 직접 실행되는 도구. `web_search`, `code_execution`, `web_fetch`, `tool_search`가 여기 속한다. 클라이언트는 결과만 받는다(별도의 `tool_result` 왕복이 필요 없다).

```python
# Server tool 사용 예 — Anthropic이 검색을 실행하고 결과를 본문에 합쳐서 돌려준다
...
이 문서의 이후 예시는 모두 client tool 기준이다.
...
### Strict tool use — 스키마 정합성 강제

도구 정의에 `"strict": true`를 추가하면 Claude의 도구 호출이 스키마와 정확히 일치하도록 강제된다. 추가 필드나 누락 없이 입력이 보장되어, 구조화된 데이터 추출 파이프라인의 안정성을 높인다.
...
자세한 제약(허용되지 않는 JSON Schema 키 등)은 [Strict tool use](https://platform.claude.com/docs/en/agents-and-tools/tool-use/strict-tool-use) 문서를 참고한다.
...
### Extended Thinking과 함께 쓸 때의 제약

Extended Thinking을 활성화한 상태에서는 `tool_choice` 옵션 중 일부가 막힌다. 공식 문서상 thinking 사용 시에는 `tool_choice: {"type": "auto"}`와 `{"type": "none"}`만 허용되고, `{"type": "any"}`나 특정 도구 강제(`{"type": "tool", "name": "..."}`)는 API 에러로 거부된다.

이유는 추론 과정과 도구 호출 결정이 모델 내부에서 맞물려 있기 때문이다. thinking이 켜진 상태에서 도구를 강제하면 추론 블록과 도구 호출의 일관성이 깨지므로, Anthropic은 아예 조합을 차단한다.

| thinking 상태 | 허용되는 `tool_choice` |
|---|---|
| 비활성화 | `auto`, `any`, `tool`, `none` 모두 가능 |
| 활성화 (Extended / Adaptive) | `auto`, `none`만 가능 |

구조화된 결과를 반드시 도구로 받아야 하는 파이프라인이라면, thinking을 끄거나 별도 호출로 분리해야 한다. 자세한 내용은 [확장 사고](05-확장-사고.md)를 참고한다.

출처: [Extended Thinking](https://platform.claude.com/docs/en/build-with-claude/extended-thinking), [Tool use 구현 가이드](https://platform.claude.com/docs/en/docs/agents-and-tools/tool-use/implement-tool-use).
...
- [How tool use works (client vs server)](https://platform.claude.com/docs/en/agents-and-tools/tool-use/how-tool-use-works)
- [Define tools / Handle tool calls](https://platform.claude.com/docs/en/agents-and-tools/tool-use/define-tools)
- [Strict tool use](https://platform.claude.com/docs/en/agents-and-tools/tool-use/strict-tool-use)
- [Token Counting for Tools](https://platform.claude.com/docs/en/build-with-claude/tool-use/token-counting)
