...
> **난이도**: 심화 | **선행 문서**: [모델 선택 가이드](04-모델-선택-가이드.md) | **최초 작성**: 2026-03-31 | **마지막 수정**: 2026-04-22
>
...
Extended Thinking은 Opus 4.6, Sonnet 4.6, Haiku 4.5에서 지원한다. Adaptive Thinking(`type: "adaptive"`)은 Opus 4.7, Opus 4.6, Sonnet 4.6에서 지원하며 Haiku 4.5는 수동 모드(`type: "enabled"`)만 사용할 수 있다.

> **주의 (Opus 4.7)**: Opus 4.7은 `thinking={"type": "enabled", "budget_tokens": N}` 형태의 수동 모드를 더 이상 지원하지 않는다. 4.7에서는 `thinking={"type": "adaptive"}`만 허용되며, 추론 깊이는 `output_config.effort`(`low`/`medium`/`high`/`xhigh`/`max`)로 제어한다. 아래의 `budget_tokens` 예시는 Opus 4.6/Sonnet 4.6/Haiku 4.5 기준이다.
...
**단축키 토글**: `Option+T` (macOS) / `Alt+T` (Windows/Linux)를 누르면 확장 사고 모드가 켜지고 꺼진다. macOS에서는 터미널에 Option을 Meta로 설정해야 동작한다. 공식 [Interactive mode](https://code.claude.com/docs/en/interactive-mode) 문서에 명시된 단축키다. 일반 작업 중에는 끄고 있다가 복잡한 리팩토링이나 아키텍처 결정이 필요할 때 켜는 방식이 실용적이다.

**프롬프트 내 키워드로 유도**: "think", "think hard", "think harder", "ultrathink" 같은 표현을 프롬프트에 포함하면 확장 사고가 강하게 유도되는 경향이 있다. 공식적으로 보장된 스위치는 아니지만, 일반적으로 관측되는 동작이다.

**자동 활성화에 대한 주의**: "Claude Code가 복잡도를 판단해 thinking을 자동으로 켠다"는 서술은 공식 Claude Code 문서에 명시되어 있지 않다. 큰 리팩토링이나 다단계 분석 요청에서 thinking이 더 자주 관찰되는 패턴은 있으나, 이것이 자동 활성화 결과인지 모델이 스스로 선택한 것인지는 사용자가 구분하기 어렵다. "자동으로 켜진다"보다 "복잡한 요청에서는 thinking이 더 자주 쓰인다" 정도로 이해하는 편이 정확하다.
